...
def genRoundKeys(key):
    ...
    for i in range(1, 33):
        ...
        key = rol(key, 61, 80)
        last_four = key >> 76
        last_four = sbox[last_four]
        key =  (last_four << 76) | (key & (2**76 - 1))
        middle = (key & (2**20 - 1)) >> 15
        middle = middle ^ i
        last_60 =  (key >> 20) << 20
        first_15 = (key & (2**15-1))
        key = last_60 | (middle << 15) | first_15
...
def sBoxLayer(state):
    ...
    for i in range(16):
        mask = 2**4 - 1
        mask_four_bits = state & mask
        output_sbox = sbox[mask_four_bits]
        
        # shift output to respective 4 bit placing
        interim_pos = (output_sbox<<(i*4))
        res = res | interim_pos
        state >>= 4
    return res
...
def sBoxLayer_inv(state):
    ...
    for i in range(16):
        mask = 2**4 - 1
        mask_four_bits = state & mask
        #get index of sbox to find inital value
        output_sbox = sbox.index(mask_four_bits)
